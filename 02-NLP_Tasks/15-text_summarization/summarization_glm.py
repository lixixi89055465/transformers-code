...
# 文件地址 ~/.cache\huggingface\modules\transformers_modules\THUDM\glm-large-chinese\
# 230f54e413fab4bc8f29bd3508aab301d757ef3e\tokenization_glm.py
# 231行 super().__init__(**kwargs) 移动至 235行，放至self.sp_model.Load(vocab_file)的后面一行

tokenizer = AutoTokenizer.from_pretrained(
    "/home/nanji/workspace/glm-large-chinese", \
...
def process_func(exmaples):
    contents = ["摘要生成: \n" + e + tokenizer.mask_token
                for e in exmaples["content"]]
    ...
    inputs = tokenizer.build_inputs_for_generation(
        inputs, \
        targets=exmaples['title'], \
        padding=True, max_gen_length=64
    )
    return inputs
...
print(t1)
t2 = tokenized_ds["train"][0]["labels"]
print(t2)
...
scores = rouge.get_scores(docode_preds, \
                          decode_labels, \
                          avg=True)
