...
def process_function(examples):
    sentences = []
    labels = []
    for sen1, sen2, label in zip(examples['sentence1'],
                                 examples['sentences2'],
                                 examples['label']):
        sentences.append(sen1)
        sentences.append(sen2)
        labels.append(1 if int(label) == 1 else -1)
    tokenized_examples = tokenizer(sentences,
                                   max_length=128,
                                   truncation=True,
                                   padding='max_length')
    tokenized_examples = {
...
train_args = TrainingArguments(
    ...
    per_device_train_batch_size=32,  # 训练时的batch
    per_device_eval_batch_size=32,  # 验证时的batch
    logging_steps=10,  # log 打印的频率
    eval_strategy='epoch',  # 评估策略
    save_strategy='epoch',  # 保存策略
    save_total_limit=3,  # 最大的保存数
